...
open class DropDownTextField : UITextField {
    ...
    fileprivate  var dataArray = [AirportLocation]()
    ...
    var optionArray = [AirportLocation]() {
    ...
    fileprivate var didSelectCompletion: (AirportLocation, Int) -> () = {selectedValue, index in }
    ...
    func didSelect(completion: @escaping (_ selectedValue: AirportLocation, _ index: Int) -> ()) {
...
extension DropDownTextField: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = "\(dataArray[indexPath.row].name) (\(dataArray[indexPath.row].code))"
        ...
        cell?.textLabel?.textColor = #colorLiteral(red: 0.2431372549, green: 0.3529411765, blue: 0.4078431373, alpha: 1)
...
extension DropDownTextField: UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let selectedValue = self.dataArray[self.selectedIndex!]
        ...
        UIView.animate(withDuration: 0.5,
                       ...
                       completion: { (didFinish) -> Void in
                        self.text = "\(selectedValue.name) (\(selectedValue.code))"
        ...
        if let selected = optionArray.firstIndex(where: {$0.name == selectedValue.name}) {
            didSelectCompletion(selectedValue, selected)
        }
    }
}
